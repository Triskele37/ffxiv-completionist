import { DataGroup } from "../../../DataGroup";

export const Crafting_Log_Culinarian_Master_Recipes_Master_Recipes_7 = function(name, parent) {
    const data = new DataGroup(name, parent).initializeTasks(tasks);
    data.name = "Master Recipes (8)";
    return data;
};

const tasks = [
    {
        "level": "80★★",
        name: "Rare Roast Beef",
        "mat-1": "2 Dzo Tenderloin",
        "mat-2": "Frantoio Oil",
        "mat-3": "2 Peppermint",
        "mat-4": "2 Night Pepper",
        "mat-5": "Fennel",
        "mat-6": "2 Gyr Abanian Carrot",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Creamy Salmon Pasta",
        "mat-1": "2 King Salmon",
        "mat-2": "Frantoio Oil",
        "mat-3": "2 Mist Spinach",
        "mat-4": "2 Night Pepper",
        "mat-5": "Vermicelli",
        "mat-6": "Sweet Cream",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "King Salmon Meuniere",
        "mat-1": "2 King Salmon",
        "mat-2": "Upland Wheat Flour",
        "mat-3": "2 Lemonette",
        "mat-4": "2 Night Pepper",
        "mat-5": "Fermented Butter",
        "mat-6": "X",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Exquisite Beef Stew",
        "mat-1": "2 Dzo Tenderloin",
        "mat-2": "Upland Wheat Flour",
        "mat-3": "2 Russet Popoto",
        "mat-4": "2 Underground Spring Water",
        "mat-5": "2 Night Pepper",
        "mat-6": "2 Gyr Abanian Carrot",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Mejillones al Ajillo",
        "mat-1": "Sweetmeat Mussel",
        "mat-2": "Frantoio Oil",
        "mat-3": "Curly Parsley",
        "mat-4": "Garlean Garlic",
        "mat-5": "Chiaroglow Aethersand",
        "mat-6": "X",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Popoto Salad",
        "mat-1": "2 Robe Lettuce",
        "mat-2": "Frantoio Oil",
        "mat-3": "2 Russet Popoto",
        "mat-4": "Lorikeet Egg",
        "mat-5": "2 Night Pepper",
        "mat-6": "2 Ramhorn Zucchini",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Robe Lettuce Salad",
        "mat-1": "2 Robe Lettuce",
        "mat-2": "Frantoio Oil",
        "mat-3": "2 Mist Spinach",
        "mat-4": "Lorikeet Egg",
        "mat-5": "2 Blood Tomato",
        "mat-6": "2 Fennel",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Arros Negre",
        "mat-1": " Sweetmeat Mussel",
        "mat-2": "Squid Ink",
        "mat-3": "Bomba Rice",
        "mat-4": "Paprika",
        "mat-5": "Chiaroglow Aethersand",
        "mat-6": "X",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Pixieberry Cheesecake",
        "mat-1": "2 Pixieberry",
        "mat-2": "Garden Beet Sugar",
        "mat-3": "2 Lemonette",
        "mat-4": "2 Sour Cream",
        "mat-5": "Yak Milk",
        "mat-6": "2 Gelatin",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Kukuru Rusk",
        "mat-1": "2 Kriollo Kukuru Bean",
        "mat-2": "Upland Wheat Flour",
        "mat-3": "2 Ala Mhigan Sal Crystal",
        "mat-4": "2 Underground Spring Water",
        "mat-5": "Yak Milk",
        "mat-6": "X",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Lemon Curd Sachertorte",
        "mat-1": "2 Kriollo Kukuru Bean",
        "mat-2": "Upland Wheat Flour",
        "mat-3": "2 Lemonette",
        "mat-4": "2 Sour Cream",
        "mat-5": "Fermented Butter",
        "mat-6": "2 Royal Kukuru Bean",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
    {
        "level": "80★★",
        name: "Pixieberry Tea",
        "mat-1": "2 Pixieberry",
        "mat-2": "Garden Beet Sugar",
        "mat-3": "2 Peppermint",
        "mat-4": "2 Underground Spring Water",
        "mat-5": "Coerthan Tea Leaves",
        "mat-6": "X",
        "catalysts": "2 Fire Cluster, 2 Water Cluster",
        "special": "Unlocked w/ Master Culinarian VII"
    },
];
