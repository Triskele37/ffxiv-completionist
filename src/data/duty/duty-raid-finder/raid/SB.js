import { DataGroup } from "../../../DataGroup";

export const Duty_Raids_SB = function(parent) {
    return new DataGroup("SB", parent).initializeTasks(tasks);
};

const tasks = [
    {
    	level: "70",
    	iLvlReq: "295",
    	name: "Deltascape V1.0",
    	roulette: "Normal Raids",
    	unlock: "Into the Deltascape"
    }, {
    	level: "70",
    	iLvlReq: "295",
    	name: "Deltascape V2.0",
    	roulette: "Normal Raids",
    	unlock: "A Catastrophe Waiting"
    }, {
    	level: "70",
    	iLvlReq: "295",
    	name: "Deltascape V3.0",
    	roulette: "Normal Raids",
    	unlock: "The Croak Queen"
    }, {
    	level: "70",
    	iLvlReq: "295",
    	name: "Deltascape V4.0",
    	roulette: "Normal Raids",
    	unlock: "A Void at All Costs"
    }, {
    	level: "70",
    	iLvlReq: "310",
    	name: "Deltascape V1.0 (Savage)",
    	roulette: "-",
    	unlock: "The Anomaly"
    }, {
    	level: "70",
    	iLvlReq: "315",
    	name: "Deltascape V2.0 (Savage)",
    	roulette: "-",
    	unlock: "The Anomaly"
    }, {
    	level: "70",
    	iLvlReq: "320",
    	name: "Deltascape V3.0 (Savage)",
    	roulette: "-",
    	unlock: "The Anomaly"
    }, {
    	level: "70",
    	iLvlReq: "320",
    	name: "Deltascape V4.0 (Savage)",
    	roulette: "-",
    	unlock: "The Anomaly"
    }, {
    	level: "70",
    	iLvlReq: "320",
    	name: "The Unending Coil of Bahamut (Ultimate)",
    	roulette: "-",
    	unlock: "Deltascape V4.0 (Savage)"
    }, {
    	level: "70",
    	iLvlReq: "325",
    	name: "Sigmascape V1.0",
    	roulette: "Normal Raids",
    	unlock: "No Slowing Down"
    }, {
    	level: "70",
    	iLvlReq: "325",
    	name: "Sigmascape V2.0",
    	roulette: "Normal Raids",
    	unlock: "An Unfinished Masterpiece"
    }, {
    	level: "70",
    	iLvlReq: "325",
    	name: "Sigmascape V3.0",
    	roulette: "Normal Raids",
    	unlock: "Won't Let You Pass"
    }, {
    	level: "70",
    	iLvlReq: "325",
    	name: "Sigmascape V4.0",
    	roulette: "Normal Raids",
    	unlock: "Test World of Ruin"
    }, {
    	level: "70",
    	iLvlReq: "340",
    	name: "Sigmascape V1.0 (Savage)",
    	roulette: "-",
    	unlock: "Test World of Ruin"
    }, {
    	level: "70",
    	iLvlReq: "345",
    	name: "Sigmascape V2.0 (Savage)",
    	roulette: "-",
    	unlock: "Test World of Ruin"
    }, {
    	level: "70",
    	iLvlReq: "350",
    	name: "Sigmascape V3.0 (Savage)",
    	roulette: "-",
    	unlock: "Test World of Ruin"
    }, {
    	level: "70",
    	iLvlReq: "350",
    	name: "Sigmascape V4.0 (Savage)",
    	roulette: "-",
    	unlock: "Test World of Ruin"
    }, {
    	level: "70",
    	iLvlReq: "370",
    	name: "The Weapon's Refrain (Ultimate)",
    	roulette: "-",
    	unlock: "Sigmascape V4.0 (Savage)"
    }, {
    	level: "70",
    	iLvlReq: "355",
    	name: "Alphascape V1.0",
    	roulette: "Normal Raids",
    	unlock: "In the Beginning, There Was Chaos"
    }, {
    	level: "70",
    	iLvlReq: "355",
    	name: "Alphascape V2.0",
    	roulette: "Normal Raids",
    	unlock: "And Like Fire Was His Mane"
    }, {
    	level: "70",
    	iLvlReq: "355",
    	name: "Alphascape V3.0",
    	roulette: "Normal Raids",
    	unlock: "In the End, There is Omega"
    }, {
    	level: "70",
    	iLvlReq: "355",
    	name: "Alphascape V4.0",
    	roulette: "Normal Raids",
    	unlock: "To Kweh Under Distant Skies"
    }, {
    	level: "70",
    	iLvlReq: "370",
    	name: "Alphascape V1.0 (Savage)",
    	roulette: "-",
    	unlock: "To Kweh Under Distant Skies"
    }, {
    	level: "70",
    	iLvlReq: "375",
    	name: "Alphascape V2.0 (Savage)",
    	roulette: "-",
    	unlock: "To Kweh Under Distant Skies"
    }, {
    	level: "70",
    	iLvlReq: "380",
    	name: "Alphascape V3.0 (Savage)",
    	roulette: "-",
    	unlock: "To Kweh Under Distant Skies"
    }, {
    	level: "70",
    	iLvlReq: "380",
    	name: "Alphascape V4.0 (Savage)",
    	roulette: "-",
    	unlock: "To Kweh Under Distant Skies"
    }, {
    	level: "70",
    	iLvlReq: "305",
    	name: "The Royal City of Rabanastre",
    	roulette: "Alliance Raids",
    	unlock: "A City Fallen"
    }, {
    	level: "70",
    	iLvlReq: "335",
    	name: "The Ridorana Lighthouse",
    	roulette: "Alliance Raids",
    	unlock: "Annihilation"
    }, {
    	level: "70",
    	iLvlReq: "365",
    	name: "The Orbonne Monastery",
    	roulette: "Alliance Raids",
    	unlock: "The City of Lost Angels"
    }
];
