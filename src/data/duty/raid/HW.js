import { DataGroup } from "../../DataGroup";

export const Duty_Raids_HW = function(parent) {
    return new DataGroup("HW", parent).initializeTasks(tasks);
};

const tasks = [
    {
    	level: "60",
    	iLvlReq: "170",
    	name: "Alexander - The Fist of the Father",
    	roulette: "Normal Raids",
    	unlock: "Disarmed"
    }, {
    	level: "60",
    	iLvlReq: "170",
    	name: "Alexander - The Cuff of the Father",
    	roulette: "Normal Raids",
    	unlock: "Steel and Steam"
    }, {
    	level: "60",
    	iLvlReq: "170",
    	name: "Alexander - The Arm of the Father",
    	roulette: "Normal Raids",
    	unlock: "Tinker, Seeker, Soldier, Spy"
    }, {
    	level: "60",
    	iLvlReq: "170",
    	name: "Alexander - The Burden of the Father",
    	roulette: "Normal Raids",
    	unlock: "The Pulsing Heart"
    }, {
    	level: "60",
    	iLvlReq: "190",
    	name: "Alexander - The Fist of the Father (Savage)",
    	roulette: "-",
    	unlock: "A Song of Steam and Steel"
    }, {
    	level: "60",
    	iLvlReq: "195",
    	name: "Alexander - The Cuff of the Father (Savage)",
    	roulette: "-",
    	unlock: "A Song of Steam and Steel"
    }, {
    	level: "60",
    	iLvlReq: "200",
    	name: "Alexander - The Arm of the Father (Savage)",
    	roulette: "-",
    	unlock: "A Song of Steam and Steel"
    }, {
    	level: "60",
    	iLvlReq: "205",
    	name: "Alexander - The Burden of the Father (Savage)",
    	roulette: "-",
    	unlock: "A Song of Steam and Steel"
    }, {
    	level: "60",
    	iLvlReq: "200",
    	name: "Alexander - The Fist of the Son",
    	roulette: "Normal Raids",
    	unlock: "Rearmed"
    }, {
    	level: "60",
    	iLvlReq: "200",
    	name: "Alexander - The Cuff of the Son",
    	roulette: "Normal Raids",
    	unlock: "The Folly of Youth"
    }, {
    	level: "60",
    	iLvlReq: "200",
    	name: "Alexander - The Arm of the Son",
    	roulette: "Normal Raids",
    	unlock: "Toppling the Tyrant"
    }, {
    	level: "60",
    	iLvlReq: "200",
    	name: "Alexander - The Burden of the Son",
    	roulette: "Normal Raids",
    	unlock: "One Step Behind"
    }, {
    	level: "60",
    	iLvlReq: "215",
    	name: "Alexander - The Fist of the Son (Savage)",
    	roulette: "-",
    	unlock: "A Refrain for the Undaunted"
    }, {
    	level: "60",
    	iLvlReq: "220",
    	name: "Alexander - The Cuff of the Son (Savage)",
    	roulette: "-",
    	unlock: "A Refrain for the Undaunted"
    }, {
    	level: "60",
    	iLvlReq: "225",
    	name: "Alexander - The Arm of the Son (Savage)",
    	roulette: "-",
    	unlock: "A Refrain for the Undaunted"
    }, {
    	level: "60",
    	iLvlReq: "225",
    	name: "Alexander - The Burden of the Son (Savage)",
    	roulette: "-",
    	unlock: "A Refrain for the Undaunted"
    }, {
    	level: "60",
    	iLvlReq: "230",
    	name: "Alexander - The Eyes of the Creator",
    	roulette: "Normal Raids",
    	unlock: "The Coeurl and the Colossus"
    }, {
    	level: "60",
    	iLvlReq: "230",
    	name: "Alexander - The Breath of the Creator",
    	roulette: "Normal Raids",
    	unlock: "Biggs and Wedge's Excellent Adventure"
    }, {
    	level: "60",
    	iLvlReq: "230",
    	name: "Alexander - The Heart of the Creator",
    	roulette: "Normal Raids",
    	unlock: "Thus Spake Quickthinx"
    }, {
    	level: "60",
    	iLvlReq: "230",
    	name: "Alexander - The Soul of the Creator",
    	roulette: "Normal Raids",
    	unlock: "Judgment Day"
    }, {
    	level: "60",
    	iLvlReq: "245",
    	name: "Alexander - The Eyes of the Creator (Savage)",
    	roulette: "-",
    	unlock: "-"
    }, {
    	level: "60",
    	iLvlReq: "250",
    	name: "Alexander - The Breath of the Creator (Savage)",
    	roulette: "-",
    	unlock: "-"
    }, {
    	level: "60",
    	iLvlReq: "255",
    	name: "Alexander - The Heart of the Creator (Savage)",
    	roulette: "-",
    	unlock: "-"
    }, {
    	level: "60",
    	iLvlReq: "255",
    	name: "Alexander - The Soul of the Creator (Savage)",
    	roulette: "-",
    	unlock: "-"
    }, {
    	level: "60",
    	iLvlReq: "175",
    	name: "The Void Ark",
    	roulette: "Alliance Raids",
    	unlock: "To Rule the Skies"
    }, {
    	level: "60",
    	iLvlReq: "205",
    	name: "The Weeping City of Mhach",
    	roulette: "Alliance Raids",
    	unlock: "The Weeping City"
    }, {
    	level: "60",
    	iLvlReq: "235",
    	name: "Dun Scaith",
    	roulette: "Alliance Raids",
    	unlock: "Where Shadows Reign"
    }
];
