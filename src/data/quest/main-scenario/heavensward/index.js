import { QuestColumnConfig } from "../../columnConfigs";
import { ArtoirelQuests } from "./artoirel";
import { EmmanellainQuests } from "./emmanellain";

// https://xivapi.com/search?indexes=Quest&filters=JournalGenreTargetID=3&columns=ID,Name&limit=94
export const HeavenswardQuests = {
    name: "Heavensward",
    subGroups: [
        ArtoirelQuests,
        EmmanellainQuests
    ],
	columns: QuestColumnConfig,
    tasks: [
        {
        	level: "50",
        	name: "Coming to Ishgard",
        	npc: "Alphinaud Leveilleur",
        	unlock: "Ishgard"
        }, {
        	level: "50",
        	name: "Taking in the Sights",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "50",
        	name: "The Better Half",
        	npc: "House Fortemps Manservant",
        	unlock: ""
        }, {
        	level: "50",
        	name: "Divine Intervention",
        	npc: "House Fortemps Steward",
        	unlock: "Aether Compass"
        }, {
        	level: "50",
        	name: "Disclosure",
        	npc: "Haurchefant Greystone",
        	unlock: ""
        }, {
        	level: "51",
        	name: "Flame General Affairs",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "51",
        	name: "In Search of Raubahn",
        	npc: "Higiri",
        	unlock: ""
        }, {
        	level: "51",
        	name: "Keeping the Flame Alive",
        	npc: "Hozan",
        	unlock: ""
        }, {
        	level: "51",
        	name: "To Siege or Not to Siege",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "51",
        	name: "Alphinaud's Way",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "51",
        	name: "In Search of Iceheart",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "51",
        	name: "From One Heretic to Another",
        	npc: "Expedition Leader",
        	unlock: ""
        }, {
        	level: "51",
        	name: "Sounding Out the Amphitheatre",
        	npc: "Alphinaud Leveilleur",
        	unlock: "Mount Speed Increase"
        }, {
        	level: "51",
        	name: "Camp of the Convictors",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "51",
        	name: "Purple Flame, Purple Flame",
        	npc: "Estinien Wyrmblood",
        	unlock: "Aether Current"
        }, {
        	level: "52",
        	name: "Where the Chocobos Roam",
        	npc: "Ysayle",
        	unlock: "The Dravanian Forelands"
        }, {
        	level: "52",
        	name: "Worse than Dragons",
        	npc: "Marcechamp",
        	unlock: ""
        }, {
        	level: "52",
        	name: "The Trine Towers",
        	npc: "Ysayle",
        	unlock: ""
        }, {
        	level: "52",
        	name: "Gifts for the Outcasts",
        	npc: "Ysayle",
        	unlock: ""
        }, {
        	level: "52",
        	name: "The Nonmind",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "53",
        	name: "A Gnathic Deity",
        	npc: "Ysayle",
        	unlock: ""
        }, {
        	level: "53",
        	name: "Breaking into Hives",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "53",
        	name: "Lord of the Hive",
        	npc: "Ysayle",
        	unlock: "Thok ast Thok (Hard), Mount Speed Increase"
        }, {
        	level: "53",
        	name: "Mourn in Passing",
        	npc: "Alphinaud Leveilleur",
        	unlock: "The Churning Mists, Sohm Al, Aether Current"
        }, {
        	level: "53",
        	name: "Beyond the Clouds",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "54",
        	name: "Mountaintop Diplomacy",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "54",
        	name: "Moghan's Trial",
        	npc: "Moghan",
        	unlock: ""
        }, {
        	level: "54",
        	name: "Mogmug's Trial",
        	npc: "Mogmug",
        	unlock: ""
        }, {
        	level: "54",
        	name: "Mogwin's Trial",
        	npc: "Mogwin",
        	unlock: ""
        }, {
        	level: "54",
        	name: "Moglin's Judgment",
        	npc: "Moglin",
        	unlock: ""
        }, {
        	level: "54",
        	name: "Leaving Moghome",
        	npc: "Moghan",
        	unlock: ""
        }, {
        	level: "54",
        	name: "The Road to Zenith",
        	npc: "Moghan",
        	unlock: ""
        }, {
        	level: "54",
        	name: "Waiting for the Wind to Change",
        	npc: "Moghan",
        	unlock: "Mount Speed Increase"
        }, {
        	level: "54",
        	name: "Heart of Ice",
        	npc: "Moghan",
        	unlock: ""
        }, {
        	level: "54",
        	name: "The Wyrm's Lair",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "54",
        	name: "New Winds, Old Friends",
        	npc: "Estinien Wyrmblood",
        	unlock: ""
        }, {
        	level: "54",
        	name: "A General Summons",
        	npc: "Tataru",
        	unlock: ""
        }, {
        	level: "54",
        	name: "Awakening in Ul'dah",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "54",
        	name: "A Brave Resolution",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "55",
        	name: "Ready to Fly",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "55",
        	name: "Into the Aery",
        	npc: "Estinien Wyrmblood",
        	unlock: "The Aery, Aether Current"
        }, {
        	level: "55",
        	name: "The Song Begins",
        	npc: "Estinien Wyrmblood",
        	unlock: ""
        }, {
        	level: "56",
        	name: "Unrest in Ishgard",
        	npc: "Estinien Wyrmblood",
        	unlock: ""
        }, {
        	level: "56",
        	name: "He Who Would Not Be Denied",
        	npc: "Haurchefant Greystone",
        	unlock: ""
        }, {
        	level: "56",
        	name: "Ill-weather Friends",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "56",
        	name: "The Spice of Life",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "56",
        	name: "Noble Indiscretions",
        	npc: "Gibrillont",
        	unlock: ""
        }, {
        	level: "56",
        	name: "A Child Apart",
        	npc: "Gibrillont",
        	unlock: ""
        }, {
        	level: "56",
        	name: "Bloodlines",
        	npc: "Gibrillont",
        	unlock: ""
        }, {
        	level: "57",
        	name: "Fire and Blood",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "57",
        	name: "A Knight's Calling",
        	npc: "Hilda",
        	unlock: "The Vault"
        }, {
        	level: "57",
        	name: "The Sins of Antiquity",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "57",
        	name: "In Search of the Soleil",
        	npc: "Lucia goe Junius",
        	unlock: ""
        }, {
        	level: "57",
        	name: "Into the Blue",
        	npc: "Alphinaud Leveilleur",
        	unlock: "Ok' Zundu"
        }, {
        	level: "57",
        	name: "Familiar Faces",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "57",
        	name: "Devourer of Worlds",
        	npc: "Lonu Vanu",
        	unlock: ""
        }, {
        	level: "57",
        	name: "Black and the White",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "57",
        	name: "Bolt, Chain, and Island",
        	npc: "Alphinaud Leveilleur",
        	unlock: "The Limitless Blue (Hard), Aether Current, Mount Speed Increase"
        }, {
        	level: "57",
        	name: "A Difference of Opinion",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "57",
        	name: "One Good Turn",
        	npc: "Cid Garlond",
        	unlock: ""
        }, {
        	level: "57",
        	name: "An Engineering Enterprise",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "57",
        	name: "Aetherial Trail",
        	npc: "Tataru",
        	unlock: ""
        }, {
        	level: "57",
        	name: "Lost in the Lifestream",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "57",
        	name: "Tataru's Surprise",
        	npc: "Tataru",
        	unlock: ""
        }, {
        	level: "57",
        	name: "Onward to Sharlayan",
        	npc: "Y'shtola",
        	unlock: ""
        }, {
        	level: "58",
        	name: "A Great New Nation",
        	npc: "Y'shtola",
        	unlock: "Idyllshire, The Dravanian Hinterlands"
        }, {
        	level: "58",
        	name: "Golems Begone",
        	npc: "Slowfix",
        	unlock: ""
        }, {
        	level: "58",
        	name: "An Illuminati Incident",
        	npc: "Slowfix",
        	unlock: ""
        }, {
        	level: "58",
        	name: "Leaving Idyllshire",
        	npc: "Slowfix",
        	unlock: ""
        }, {
        	level: "59",
        	name: "Matoya's Cave",
        	npc: "Y'shtola",
        	unlock: "Matoya's Cave (Zone), Mount Speed Increase"
        }, {
        	level: "59",
        	name: "Forbidden Knowledge",
        	npc: "Matoya",
        	unlock: "The Great Gubal Library, Aether Current"
        }, {
        	level: "59",
        	name: "An Eye for Aether",
        	npc: "Matoya",
        	unlock: ""
        }, {
        	level: "59",
        	name: "Hour of Departure",
        	npc: "Alphinaud Leveilleur",
        	unlock: ""
        }, {
        	level: "59",
        	name: "The First Flight of the Excelsior",
        	npc: "Alphinaud Leveilleur",
        	unlock: "Azys Lla"
        }, {
        	level: "59",
        	name: "Systematic Exploration",
        	npc: "Cid Garlond",
        	unlock: "Aether Current"
        }, {
        	level: "59",
        	name: "In Node We Trust",
        	npc: "Guidance Node",
        	unlock: "Aether Current"
        }, {
        	level: "59",
        	name: "Chimerical Maintenance",
        	npc: "Guidance Node",
        	unlock: "Aether Current"
        }, {
        	level: "59",
        	name: "Close Encounters of the VIth Kind",
        	npc: "Guidance Node",
        	unlock: "Aether Current, Mount Speed Increase"
        }, {
        	level: "59",
        	name: "Fetters of Lament",
        	npc: "Guidance Node",
        	unlock: "Aether Current"
        }, {
        	level: "60",
        	name: "Heavensward",
        	npc: "Guidance Node",
        	unlock: "The Aetherochemical Research Facility, The Singularity Reactor, Mount Speed Increase"
        },
    ]
};
