import { DataGroup } from "../../../DataGroup";
import { FishingGuideColumnConfig } from "../../columnConfigs";

export const Gathering_Log_Fishing_Guide_Page_8 = function(parent) {
    return new DataGroup("Page 8", parent).initializeTasks(tasks, FishingGuideColumnConfig);
};

const tasks = [
    {
        name: "Emperor Fish",
        iLvl: "55",
        primeLocation: "Coerthas Central Highlands",
        fishingHoles: "The Nail",
        type: "Rivers",
        bait: "Abalathian Smelt",
        usage: "Cooking",
        special: "Mooch"
    }, {
        name: "Bowfin",
        iLvl: "55",
        primeLocation: "North Shroud",
        fishingHoles: "Lake Tahtotl",
        type: "Lakes",
        bait: "Goldfish",
        usage: "N/A",
        special: "Mooch"
    }, {
        name: "Heliobatis",
        iLvl: "70",
        primeLocation: "Mor Dhona",
        fishingHoles: "North Silvertear",
        type: "Rivers",
        bait: "Caddisfly Larva",
        usage: "N/A",
        special: "5PM-8AM"
    }, {
        name: "Megalodon",
        iLvl: "70",
        primeLocation: "Western La Noscea",
        fishingHoles: "The Ship Graveyard",
        type: "Deep Sea",
        bait: "Wahoo",
        usage: "Cooking, Goldsmithing",
        special: "Mooch"
    }, {
        name: "Titanic Sawfish",
        iLvl: "70",
        primeLocation: "Western Thanalan",
        fishingHoles: "Cape Westwind",
        type: "Deep Sea",
        bait: "Wahoo",
        usage: "Quest",
        special: "9AM-3PM, Clear/Fair"
    }, {
        name: "Takitaro",
        iLvl: "70",
        primeLocation: "South Shroud",
        fishingHoles: "Urth's Gift",
        type: "Lakes",
        bait: "Streamer",
        usage: "Alchemy",
        special: "N/A"
    }, {
        name: "Pirarucu",
        iLvl: "70",
        primeLocation: "North Shroud",
        fishingHoles: "Proud Creek",
        type: "Lakes",
        bait: "Streamer",
        usage: "N/A",
        special: "N/A"
    }, {
        name: "Morinabaligi",
        iLvl: "70",
        primeLocation: "Mor Dhona",
        fishingHoles: "The North Shards",
        type: "Rivers",
        bait: "Goldfish",
        usage: "N/A",
        special: "Mooch"
    }, {
        name: "Jungle Catfish",
        iLvl: "55",
        primeLocation: "Eastern La Noscea",
        fishingHoles: "Red Mantis Falls",
        type: "Rivers",
        bait: "Goldfish",
        usage: "N/A",
        special: "Mooch"
    }, {
        name: "Tiny Tortoise",
        iLvl: "50",
        primeLocation: "Lower La Noscea",
        fishingHoles: "The Salt Strand",
        type: "Coastlines",
        bait: "Yumizuno",
        usage: "Minion",
        special: "N/A"
    }, {
        name: "Gigantpole",
        iLvl: "50",
        primeLocation: "Eastern Thanalan",
        fishingHoles: "The Burning Wall",
        type: "Lakes",
        bait: "Topwater Frog",
        usage: "Minion",
        special: "N/A"
    }, {
        name: "Zalera",
        iLvl: "70",
        primeLocation: "Middle La Noscea",
        fishingHoles: "Zephyr Drift",
        type: "Deep Sea",
        bait: "Rat Tail",
        usage: "N/A",
        special: "314+ GAT, 9AM-2PM"
    }, {
        name: "Beguiler Chub",
        iLvl: "70",
        primeLocation: "Middle La Noscea",
        fishingHoles: "Rogue River",
        type: "Rivers",
        bait: "Moth Pupa",
        usage: "N/A",
        special: "9AM-2PM"
    }, {
        name: "Oschon's Print",
        iLvl: "70",
        primeLocation: "Lower La Noscea",
        fishingHoles: "Moraby Bay",
        type: "Coastlines",
        bait: "Goby Ball",
        usage: "N/A",
        special: "308+ GAT"
    }, {
        name: "Caterwauler",
        iLvl: "70",
        primeLocation: "Lower La Noscea",
        fishingHoles: "The Mourning Widow",
        type: "Rivers",
        bait: "Moth Pupa",
        usage: "N/A",
        special: "293+ GAT"
    }, {
        name: "Crystal Perch",
        iLvl: "80",
        primeLocation: "Middle La Noscea",
        fishingHoles: "Woad Whisper Canyon",
        type: "Rivers",
        bait: "Butterworm",
        usage: "N/A",
        special: "345+ GAT, Overcast/Fog/Wind"
    }, {
        name: "Moldva",
        iLvl: "80",
        primeLocation: "East Shroud",
        fishingHoles: "Springripple Brook",
        type: "Lakes",
        bait: "Ala Mhigan Fighting Fish",
        usage: "N/A",
        special: "Mooch, 335+ GAT, 6PM-2AM"
    }, {
        name: "Junkmonger",
        iLvl: "80",
        primeLocation: "Western La Noscea",
        fishingHoles: "Skull Valley",
        type: "Deep Sea",
        bait: "Wahoo",
        usage: "N/A",
        special: "Mooch, 339+ GAT, 4PM-2AM"
    }, {
        name: "Goldenfin",
        iLvl: "70",
        primeLocation: "Limsa Lominsa",
        fishingHoles: "Limsa Lominsa Upper Decks",
        type: "Coastlines",
        bait: "Pill Bug",
        usage: "N/A",
        special: "9AM-2PM"
    }, {
        name: "Gigantshark",
        iLvl: "70",
        primeLocation: "Middle La Noscea",
        fishingHoles: "Summerford",
        type: "Deep Sea",
        bait: "Harbor Herring",
        usage: "N/A",
        special: "Mooch, 299+ GAT, Clear/Fair/Overcast"
    }, {
        name: "Armorer",
        iLvl: "75",
        primeLocation: "Western La Noscea",
        fishingHoles: "Swiftperch",
        type: "Deep Sea",
        bait: "Northern Krill",
        usage: "N/A",
        special: "7PM-3AM"
    }, {
        name: "Great Gudgeon",
        iLvl: "70",
        primeLocation: "Middle La Noscea",
        fishingHoles: "West Agelyss River",
        type: "Rivers",
        bait: "Crayfish Ball",
        usage: "N/A",
        special: "306+ GAT"
    }, {
        name: "Dark Knight",
        iLvl: "80",
        primeLocation: "Central Thanalan",
        fishingHoles: "The Unholy Heir",
        type: "Lakes",
        bait: "Crow Fly",
        usage: "N/A",
        special: "305+ GAT, Overcast/Fog/Dust Storm"
    }, {
        name: "Silver Sovereign",
        iLvl: "70",
        primeLocation: "Lower La Noscea",
        fishingHoles: "Oschon's Torch",
        type: "Deep Sea",
        bait: "Yumizuno",
        usage: "Trade",
        special: "290+ GAT"
    }, {
        name: "Sabertooth Cod",
        iLvl: "70",
        primeLocation: "Lower La Noscea",
        fishingHoles: "Candlekeep Quay",
        type: "Coastlines",
        bait: "Goby Ball",
        usage: "N/A",
        special: "345+ GAT, 4PM-10PM"
    },
];
