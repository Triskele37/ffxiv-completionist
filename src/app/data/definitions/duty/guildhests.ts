...
export function Duty_Guildhests(translate: TranslateService) {
    return (parent: DataGroup, rootKey: string) => {
        const group = fromJson(parent, `${rootKey}`);
        group.subGroups = new Map();

        const addGuildhestClone = (key: string, name: string) => {
            const clone = fromMappedJson(group, `${rootKey}.template`, (json) => {
                json.groupName = name;
                json.key = key;
                return json;
            });
