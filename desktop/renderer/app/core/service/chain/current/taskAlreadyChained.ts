...
export function taskAlreadyChained(service: ChainService) {
    return (
        task: Task,
        toFlag: string,
    ): boolean => {
        // Matches start task
        if(service.chainStart()?.task?.fullStorageKey === task.fullStorageKey) {
            return true;
        }

        // Matches embedded chained tasks
        return service.chainedGroups().some((chainedGroup) => {
