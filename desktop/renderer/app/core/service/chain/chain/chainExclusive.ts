 ...
 * */
export function chainExclusive(service: ChainService) {
    return (
        ...
        service.svcData.getTasks(task.cExclusive, task).forEach((chainTask) => {
            if(flag === Completion.X) {
                // Chain exclusive tasks to be excluded as well
                service.applyFlagToTask(chainTask, Completion.X, true);
            }
            else if(flag === Completion.N) {
                // Un-exclude any tasks excluded due to this task if marked incomplete
                service.svcData.getTasks(task.cExclude, task).forEach((task) => {
                    if(task.completionFlag$() !== Completion.X) return;
                    service.applyFlagToTask(task, Completion.N, true);
                });

                // Chain exclusive tasks to be incomplete as well
                service.applyFlagToTask(chainTask, Completion.N, true);
            }
