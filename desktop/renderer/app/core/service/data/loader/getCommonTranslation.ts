...
export function getCommonTranslation(service: DataService) {
    /**
    ...
    return (
        ...
        while(commonKeys = updatedValue.match(COMMON_KEY_REGEX)) {
            ...
            for(const commonKey of commonKeys) {
                if(!replacements[commonKey]) {
                    ...
                    const commonTranslation = service.svcTranslate?.instant(fullCommonKey);
        ...
        if(service.svcConfig.get('lang') === 'fr') {
