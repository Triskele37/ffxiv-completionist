 ...
 * */
export function initGroup(service: DataService) {
    return (): DataGroup => {
        // Inject the full data cache into the renderer
        service.dataCache = service.svcElectron?.getData();

        // Load the root DataGroup
        const data = service.loadGroupShallow(null, '');
        if(!data) {
        ...
        data.order?.forEach((subGroupKey) => {
            ...
            const subGroup = service.loadGroupDeep(data, subGroupKey);
            if(!subGroup) return;
        ...
        // Log unloaded groups
        const unloaded = Object.keys(service.dataCache.data)
